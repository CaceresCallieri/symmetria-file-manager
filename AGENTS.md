...
**Reader routing** — The reader uses the cascade's single modal gate with its own Escape and Ctrl+Enter handler in `ReaderOverlay`, except that `useKeyDispatch` returns before the cascade for browser scroll keys on a focused reader surface with `data-scrolls="true"`. Keep it a dialog because a separate view kind would bypass modal precedence and turn a transient overlay into navigation state.

**Reader remounting** — On expand, `previewPanesFor` in `packages/fm-ui/src/App.tsx` transfers preview ownership to the reader (`Reading…` until the described path matches the cursor), and `PreviewSlot` in `MillerColumns.tsx` replaces the column preview with its reserved placeholder. Expect the document `<embed>` to lose its page position because React remounts it across these component trees instead of moving the same mounted node.

**Reader styling** — Reader-only layout rules stay under `.reader` or `.preview-pane--reader`, while the global `object-fit: contain` image and video rules keep both preview surfaces uncropped. Scope new reader adjustments the same way, or a reader fix will change the column and a contain rule will incorrectly become reader-only.
