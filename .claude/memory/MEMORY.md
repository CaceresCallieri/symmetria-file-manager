...
- [v2 BACKLOG (2026-08-30)](project_electron_v2_backlog.md) _(uncommitted — on disk only)_ — the ten things the operator wants NEXT, in their priority order, after trying v1. To be planned as a run of many phases, one fix per phase. The fuzzy finder is deliberately deferred to be designed with Mesura Code.
- [Electron v1 BUILT (2026-08-30)](project_electron_v1_built.md) _(uncommitted — on disk only)_ — the ten-phase plan is done and committed on `t3code/map-electron-file-manager` (430 tests, both gates clean). Read this before touching the Electron tree: it lists what is NOT built, and five traps that no test could find.
- [Electron transition](project_electron_transition.md) _(uncommitted — on disk only)_ — the FM moves from Qt6/QML to Electron: first standalone (resident daemon + fast windows), then embedded in **Mesura Code** (a fork of `pingdotgg/t3code`). 15k-line research dossier at `docs/electron-transition/` — read `12-synthesis.md` first. ⚠ That directory exists ONLY on branch `t3code/map-electron-file-manager`; `main` has no Electron tree. **Six decisions were left open for the user; do not settle them alone.** Four of the six are now settled — see [[project-electron-single-window]]; the fuzzy finder and the package-publishing mechanics remain. Note this REVERSES Symmetria IDE's written `tech-stack.md` verdict and supersedes the QML "stay" decision below.
