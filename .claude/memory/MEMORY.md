...
- [Single-window design (2026-09-01)](project_electron_single_window.md) — resident daemon on a NAMED Hyprland workspace `files` (not a scratchpad — the operator already rejected those); the picker is a dialog, not a second window; the packages split the Mesura Code embedding needs. Four decisions with reasoning, plus two gaps found while designing.
