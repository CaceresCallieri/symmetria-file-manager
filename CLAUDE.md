...
**QFileSystemWatcher directory watch is deaf to in-place content writes** — Qt's directory inotify mask is `IN_CREATE | IN_DELETE | IN_MOVE | IN_ATTRIB | …` but **omits `IN_MODIFY`/`IN_CLOSE_WRITE`**. So a file that grows on disk *after* it was first listed — e.g. a download streamed straight to its final name (`curl -O`, `wget`, `yt-dlp`, "Save image as") rather than via a `.part`/`.crdownload` temp + rename — emits **no `directoryChanged` signal at all**. Compounding this, `FileSystemEntry` holds an immutable `const QFileInfo` stat'd once at construction, and `FileSystemModel`'s diff historically compared only the *set of paths* (`newPaths == oldPaths` early-return), so a same-path size change was a no-op. The combined symptom was a downloaded file stuck at **0 bytes with no preview** until you re-navigated (which rebuilt the entries from a fresh scan). Fix (in `FileSystemModel`): `syncFileWatches()` adds a **per-file watch** for each file in the current directory so `fileChanged` fires on growth; `onFileChanged()` debounces (250ms throttle) into a rescan; and the background diff now also flags **modified** paths (same path, changed size/mtime) and rebuilds those entries with a fresh stat (treated as remove+add, preserving the "entries are immutable snapshots" invariant). Bounded deliberately: per-file watches are **non-recursive only** and capped at `kMaxFileWatches` (2048) to protect the inotify budget; in recursive mode the per-file stat in the scan loop is skipped entirely. The `growingFileRefreshesSize` test asserts a 0-byte file that later grows refreshes without re-navigation. Files that arrive via atomic rename (Firefox/Chrome temp-file downloads) never hit this — their final name appears via `IN_MOVED_TO` already full-size.
