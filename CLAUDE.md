...
- **Transparency model**: Solid-body model (macOS-Tahoe / GNOME-Files style). The Miller columns are **solid rounded surfaces** built in `MillerColumns.qml` (one `StyledRect` per column: `radius rounding.lg`, solid `palette.surfaceContainerLow`, hairline `overlay.subtle` border, content inset by `padding.md`, columns separated by `spacing.md` gaps — no more 1px separators). Transparency survives only in the **chrome**: the empty areas of the top/bottom bars around the clay pills, where `FmTheme.windowBackdrop` shows through. The backdrop is near-opaque black (`Qt.rgba(0, 0, 0, 0.9)`) so that show-through is a subtle hint of wallpaper, not a wash. (This replaced the older *Ghostty single-layer* model, where the backdrop was 0.6 and the whole body was transparent onto it.) Pitfalls: (1) `_transparencyLayers` stays **0.0** — the panels' internal backgrounds (`FmTheme.layer(surfaceContainerLow)`) remain transparent and *reveal* the `MillerColumns` surface behind them; the solid fill is owned by the column surfaces, NOT by raising the layer alpha (which would compound with the backdrop in the chrome gaps). (2) The middle column's surface is added **around** `FileList` in `MillerColumns.qml` (FileList is protected WIP and not edited directly) — `FileList` is inset over it. (3) Per-row **zebra striping** lives in `FileListItem.qml` as a plain un-animated `Rectangle` — even rows get a flat `overlay.subtle` gray, odd rows stay transparent (the only claymorphism in the list is the selection pill; rows are deliberately flat). Same j/k-stutter constraint as the selection highlight — keep it a plain `Rectangle`, no gradient/Behavior.
