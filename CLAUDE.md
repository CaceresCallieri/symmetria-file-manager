...
- **Overlay colors** via `FmTheme.overlay.subtle` (0.06 white) and `.emphasis` (0.10 white) — EDGE tokens, for separators, borders and small-swatch backgrounds like keycaps; plus `.zebra` (0.03 white) — the FILL token for alternating list rows. The split exists because a full-row fill and a hairline edge want different alphas over a near-black base
